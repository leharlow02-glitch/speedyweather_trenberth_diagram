...
#
# balance_color: turn a net-flux imbalance (W/m²) into a colour. `tol` is
# the +/- band treated as "balanced" - at coarse resolution the
# instantaneous surface/TOA net can easily wobble by a few W/m² without
# meaning anything, so this avoids flickering colour on noise rather than
# reflecting genuine drift.
...
# build_flux_timeseries_figure: assembles the 12-panel "all flux time
# series" grid.
#
# Pulled out of the "Show All Flux Time Series" button's callback (same
# treatment as build_flux_maps_figure below) so it can be reused both
# interactively (button click) and for `save_flux_timeseries_gif`, without
# duplicating the per-panel plotting logic.
"""
    build_flux_timeseries_figure(obs)

Build the Figure containing the 12-panel grid of full flux time series (one
panel per field in `TRENBERTH_PANELS`), each with a vertical dashed line and
marker tracking `obs.time_idx`. Dates label only the bottom row of panels,
since every panel shares the same x-axis.
"""
function build_flux_timeseries_figure(obs)
    (; ts, time_idx, nsteps, flux_series) = obs
    ts_steps = 1:nsteps
    n = nsteps
...
function build_flux_maps_figure(obs)
    ...
    for (field, title, color, row, col) in TRENBERTH_MAP_PANELS
        ...
        #
        # NOTE: this calls `transform(x)` with no explicit
        # SpectralTransform, which allocates a fresh one internally on
        # every single call. For a long run with many recorded map steps
        # that's a lot of allocation in this loop - if this ever crashes
        # with runaway allocations / a GC error on a long run, that's
        # the first thing to look at (build one SpectralTransform outside
        # this loop and pass it to `transform` explicitly instead).
...
function plot_trenberth_diagram(obs, arrows, solar_constant)
    ...
    Label(info_grid[6, 1], vals_text, fontsize = 20, halign = :center,
    ...
    on(all_btn.clicks) do _
        ...
        if !isnothing(scr) && isopen(scr)
            ...
            ts_fig = build_flux_timeseries_figure(obs)
...
# diagram - same arrows/imbalance arrows/info panel as plot_trenberth_diagram,
# but without the play/pause/slider controls and the popup click handling,
# since a GIF has no user to click on it - and record it frame-by-frame with
# Makie's `record`, stepping through every recorded timestep once.
"""
    save_trenberth_gif(obs, arrows, solar_constant; filename="trenberth.gif",
                        framerate=8, size=(1800, 1000))

Render the main Trenberth diagram (flux arrows, TOA/Surface net imbalance
arrows, and info panel, with no interactive controls) and save it as an
animated GIF, one frame per recorded timestep in `obs`.
"""
function save_trenberth_gif(obs, arrows, solar_constant;
    ...
    # above, including the Surface/TOA Net indicators), forces a render,
    # and appends that frame to the GIF.
...
# save_flux_timeseries_gif: same idea as save_trenberth_gif, but for the
# 12-panel "all flux time series" grid built by build_flux_timeseries_figure.
# Each panel's own dashed vertical line + marker sweeps across as the
# timestep advances. Does not require `obs.maps` to be populated.
"""
    save_flux_timeseries_gif(obs; filename="trenberth_timeseries.gif", framerate=24)

Save the 12-panel flux time series grid as an animated GIF, one frame per
recorded timestep in `obs`.
"""
function save_flux_timeseries_gif(obs; filename::AbstractString="trenberth_timeseries.gif",
                                   framerate::Int=24)
    ts_fig = build_flux_timeseries_figure(obs)
    record(ts_fig, filename, 1:obs.nsteps; framerate = framerate) do i
        obs.time_idx[] = i
    end
    return filename
end

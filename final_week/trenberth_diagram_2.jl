...
# a summary info panel (now including surface AND top-of-atmosphere net
# imbalance indicators), and click-to-popup time series for each arrow.
#
# UPDATED IN THIS VERSION:
#   1. Fixed a label-overlap bug: the long bullet-list Label in the info
#      panel had `tellheight = false`, which told the GridLayout to ignore
#      its real rendered height. That collapsed its row to ~0 height, so
#      every Label below it (date/step text, "Legend", the legend body)
#      got positioned as if the bullet list were empty and printed on top
#      of it. Fix: just let it report its real height (the default).
#   2. Added a Surface Net AND a TOA Net imbalance indicator, each with a
#      color (green/orange/blue) and direction glyph (▲/▼/●) that update
#      live as the animation runs. See `balance_color`/`balance_arrow`.
#   3. Added GIF export: `save_trenberth_gif` records the main animated
#      diagram to a .gif; `save_flux_maps_gif` records the 3x3 flux-maps
#      grid to a separate .gif. The maps-window-building logic used to
#      live only inside the "Show Flux Maps" button's callback; it's been
#      pulled out into `build_flux_maps_figure` so both the button and the
#      GIF exporter can use it without duplicating code.
#
...
#   display(fig)
#
#   # Optional: save GIFs instead of (or in addition to) the interactive window
#   save_trenberth_gif(obs, arrows, solar; filename="trenberth.gif")
#   save_flux_maps_gif(obs; filename="trenberth_maps.gif")   # needs obs.maps populated

using GLMakie, Observables, Dates

# ── Balance indicator helpers ──────────────────────────────────────────
#
# balance_color / balance_arrow: turn a net-flux imbalance (W/m²) into a
# visual cue. `tol` is the +/- band treated as "balanced" - at coarse
# resolution and short runs, the instantaneous surface/TOA net can easily
# wobble by a few W/m² without meaning anything, so this avoids flickering
# between colors on noise rather than reflecting genuine drift.
function balance_color(x::Real; tol::Real=5.0)
    x > tol  && return :orangered   # absorbing/retaining more than emitting -> warming
    x < -tol && return :dodgerblue  # emitting more than absorbing -> cooling
    return :seagreen                # within tolerance -> balanced
end

function balance_arrow(x::Real; tol::Real=5.0)
    x > tol  && return "▲"
    x < -tol && return "▼"
    return "●"
end
...
# build_flux_maps_figure: assembles the 3x3 grid of spatial flux heatmaps.
#
# Pulled out of the "Show Flux Maps" button's callback so it can be reused
# both interactively (button click) and for `save_flux_maps_gif` below,
# without duplicating the per-field conversion logic.
"""
    build_flux_maps_figure(obs)

Build the Figure containing the 3x3 grid of animated flux-map heatmaps
(one panel per field in `TRENBERTH_MAP_PANELS`), driven by `obs.time_idx`.
Returns `nothing` if `obs.maps` is empty (i.e. the callback that produced
`obs` didn't record map snapshots - see `record_maps` on `TrenberthCallback`).
"""
function build_flux_maps_figure(obs)
    (; maps, time_idx, nsteps) = obs
    ...
    for (field, title, color, row, col) in TRENBERTH_MAP_PANELS
        ...
        # `return_grids` docstring in trenberth_calc.jl) - smaller to
        # store than a rectangular grid, but not directly plottable. We
        # convert every recorded step to a displayable matrix once, up
        # front here (rather than lazily per-frame), for two reasons:
        #   1) it lets us compute one correct, static color range spanning
        #      the whole run, so the colorbar doesn't rescale as you scrub
        #      through time or as the GIF plays
        #   2) it makes scrubbing/recording afterwards instant, since
        #      nothing needs recomputing per frame
        # This costs n small spectral transforms per field, paid once here.
        #
        # Per-step conversion:
        #  1) transform(native_field) -> spectral coefficients
        #  2) transform(spec) with no Grid keyword -> defaults to
        #     FullGaussianGrid, i.e. a *rectangular* grid (reduced grids
        #     like the native one can't be reshaped into a matrix, which
        #     is what `heatmap` needs)
        #  3) Matrix(...) unwraps that into a plain matrix
        #  4) reverse(..., dims=2): SpeedyWeather unravels grid data
        #     ordered north -> south, and Matrix()'s column-major reshape
        #     puts that northernmost data in column 1 - but heatmap! draws
        #     column 1 at the *bottom* of the axis, so without this flip
        #     north ends up at the bottom (maps render upside down).
        ...
        # Static colorrange for this field, spanning every frame, so the
        # colorbar doesn't shift as you scrub or as the GIF plays.
...
the current numeric values plus live Surface Net and TOA Net imbalance
indicators, playback controls (slider/play/reset), a toggle for a "show all
flux time series" window, a toggle for a "show flux maps" window (only
shown if `obs.maps` is non-empty), and click-to-popup time series for
individual arrows.
...
Returns `(fig=..., cleanup=...)`. Call `display(fig)` to show it, and
`cleanup()` when done to stop the animation task and close popup windows.
"""
function plot_trenberth_diagram(obs, arrows, solar_constant)
    ...
    # ── Info panel: live numeric readout + balance indicators ───────────────
    ...
    # TOA net imbalance: global-mean insolation (solar_constant/4) minus
    # what's reflected (OSR) and emitted (OLR). The /4 converts the solar
    # constant (flux on a disk facing the sun, area πr²) into the
    # global-mean insolation (averaged over the whole sphere, area 4πr²) -
    # the standard way to compare it against OSR/OLR in a Trenberth budget.
    toa_net_obs = map((sc, p) -> sc / 4 - p.OSR - p.OLR, solar_constant, current_point)

    surface_net_color_obs = map(p -> balance_color(p.surface_net), current_point)
    surface_net_text_obs  = map(p -> "Surface Net: $(balance_arrow(p.surface_net))  $(round(Int, p.surface_net)) W/m²", current_point)

    toa_net_color_obs = map(x -> balance_color(x), toa_net_obs)
    toa_net_text_obs  = map(x -> "TOA Net: $(balance_arrow(x))  $(round(Int, x)) W/m²", toa_net_obs)

    Label(info_grid[3, 1], surface_net_text_obs, fontsize = 20, font = :bold,
          halign = :center, color = surface_net_color_obs)
    Label(info_grid[4, 1], toa_net_text_obs, fontsize = 20, font = :bold,
          halign = :center, color = toa_net_color_obs)

    albedo_text_obs = map(p -> "Albedo: $(round(p.albedo, digits=3))", current_point)
    Label(info_grid[5, 1], albedo_text_obs, fontsize = 20, halign = :center)
    ...
    # NOTE: `tellheight` intentionally left at its default (true) here.
    # Setting it to false previously made the GridLayout ignore this
    # Label's real rendered height, collapsing its row and causing every
    # row below it to overlap the tail end of this text.
    Label(info_grid[6, 1], vals_text, fontsize = 20, halign = :center, valign = :top)

    time_text = map(current_point) do p
        "$(Dates.format(p.datetime, "yyyy-mm-dd HH:MM"))\nStep $(time_idx[])/$(nsteps)"
    end
    Label(info_grid[7, 1], time_text, fontsize = 20, halign = :center, color = :navyblue)
    Label(info_grid[8, 1], "Legend", fontsize = 20, font = :bold, halign = :center)
    Label(info_grid[9, 1], """
    Yellow: Shortwave
    Red: Longwave
    Pink: Sensible heat
    ...
    ▲ warming   ▼ cooling   ● balanced (±5 W/m²)
    ...
    # if the callback didn't store any, e.g. record_maps=false).
    if !isempty(maps)
        ...
        on(maps_btn.clicks) do _
            ...
            if !isnothing(scr) && isopen(scr)
                ...
                maps_fig = build_flux_maps_figure(obs)
...
# ── GIF export ──────────────────────────────────────────────────────────
#
# save_trenberth_gif: build a static (non-interactive) copy of the main
# diagram - same arrows/info panel as plot_trenberth_diagram, but without
# the play/pause/slider controls and the popup click handling, since a GIF
# has no user to click on it - and record it frame-by-frame with Makie's
# `record`, stepping through every recorded timestep once.
"""
    save_trenberth_gif(obs, arrows, solar_constant; filename="trenberth.gif",
                        framerate=8, size=(1800, 1000))

Render the main Trenberth diagram (arrows + info panel, no interactive
controls) and save it as an animated GIF, one frame per recorded timestep
in `obs`. Does not require `obs.maps` to be populated.
"""
function save_trenberth_gif(obs, arrows, solar_constant;
                             filename::AbstractString="trenberth.gif",
                             framerate::Int=8,
                             size=(1800, 1000))
    (; current_point, time_idx, nsteps) = obs

    fig = Figure(; size, backgroundcolor = :white, fontsize = 40)
    ax = Axis(fig[1, 1],
        aspect          = DataAspect(),
        backgroundcolor = (:lightblue, 0.12),
        title           = "Energy Budget Diagram")
    hidedecorations!(ax); hidespines!(ax)
    xlims!(ax, -4.0, 4.0); ylims!(ax, -2.5, 3.2)
    ...
    # Record: `record` calls the do-block once per element of the given
    # iterator, sets `time_idx[]` (which cascades to every Observable
    # above), forces a render, and appends that frame to the GIF.
    record(fig, filename, 1:nsteps; framerate = framerate) do i
        time_idx[] = i
    end
...
# save_flux_maps_gif: same idea as save_trenberth_gif, but for the 3x3
# flux-maps grid instead of the arrow diagram. Requires `obs.maps` to be
# populated (i.e. the run that produced `obs` used `record_maps=true`).
"""
    save_flux_maps_gif(obs; filename="trenberth_maps.gif", framerate=4)

Save the 3x3 flux-maps grid as an animated GIF, one frame per recorded
timestep. Returns `nothing` (and warns) if `obs.maps` is empty.
"""
function save_flux_maps_gif(obs; filename::AbstractString="trenberth_maps.gif",
                             framerate::Int=4)
    if isempty(obs.maps)
        @warn "save_flux_maps_gif: obs.maps is empty - re-run with record_maps=true " *
              "on the TrenberthCallback to enable map export."
        return nothing
    end

    maps_fig = build_flux_maps_figure(obs)
    record(maps_fig, filename, 1:obs.nsteps; framerate = framerate) do i
        obs.time_idx[] = i
    end
    return filename
end
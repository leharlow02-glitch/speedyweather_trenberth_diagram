#!/usr/bin/env julia
# trenberth_glmakie_anim.jl
# GLMakie adaptation of your CairoMakie script. Produces PNG frames and attempts GIF/MP4 stitch.

using GLMakie, Colors, CSV, DataFrames, Dates, Printf, Statistics, FileIO
...
# Protruding head implementation (clean, recommended)
function draw_arrow!(ax, x1, y1, x2, y2;
    ...
    ux, uy = dx / L, dy / L
    # head scales with linewidth so it remains visible
    headsize_eff = max(headsize, linewidth * 1.6)
    # compute head_space and clamp so head has room
    head_space = clamp(headsize_eff * 1.05, linewidth*1.4, L*0.8) * head_ratio
    sx = x2 - ux * head_space
    sy = y2 - uy * head_space

    # draw shaft (butt cap so it doesn't overhang)
...
function bezier_cubic(p0, p1, p2, p3, n=48)
...
# -----------------------------
# Input CSV or create demo timeseries
# -----------------------------
...
# fixed endpoints to avoid jumpiness
y_top = atm_y + 24
y_surface_pen = surf_y + 14
y_down_start = atm_y + Int(round(atm_h*0.42))
y_down_end   = y_surface_pen
y_up_start   = y_surface_pen
y_up_end     = atm_y + Int(round(atm_h*0.45))
y_sens_end   = atm_y + Int(round(atm_h*0.50))
y_lat_end    = atm_y + Int(round(atm_h*0.50))
...
# -----------------------------
# Precompute surface Bezier polygon
# -----------------------------
...
function draw_on_axis!(ax, i)
    ...
    # columns
    for j in 1:4
        bx, tx = col_bottom_x[j], col_top_x[j]
        w = thin_w
        x1, y1 = tx, atm_y + 14
        x2, y2 = tx + w, atm_y + 14
        x3, y3 = bx + w, atm_y + atm_h - 14
        x4, y4 = bx, atm_y + atm_h - 14
    ...
    draw_arrow!(ax, cx1_bot, y_up_start, cx1_top + s, y_up_end, color = lw_col, linewidth = lw_stroke)
    text!(ax, "LW Up " * fmt(lw_up), position = (cx1_top + s - 44, y_up_end - 6), align = (:right, :center), color = textcol, fontsize = 13)

    # LW Down
    cx2_top = col_top_x[2] + thin_w*0.5 - s
    cx2_bot = col_bottom_x[2] + thin_w*0.3 - s
    draw_arrow!(ax, cx2_top - s, y_down_start, cx2_bot - 4, y_down_end, color = lw_col, linewidth = lwd_stroke)
    text!(ax, "LW Down " * fmt(lw_down), position = (cx2_bot - 46, y_down_end + 16), align = (:right, :center), color = textcol, fontsize = 13)

    # SW Down / Absorbed
    cx3_top = col_top_x[3] + thin_w*0.5 - 6
    cx3_bot = col_bottom_x[3] + thin_w*0.6 - 12
    draw_arrow!(ax, cx3_top - 6, y_down_start, cx3_bot - 12, y_down_end + 2, color = sw_col, linewidth = sw_stroke)
    text!(ax, "SW Absorbed " * fmt(sw_down - sw_ref), position = (cx3_bot - 6, y_down_end + 24), align = (:right, :center), color = textcol, fontsize = 13)

    # SW Reflected
    cx4_bot = col_bottom_x[4] + thin_w*0.5 + 6
    cx4_top = col_top_x[4] + thin_w*0.5 + 10
    draw_arrow!(ax, cx4_bot + 2, y_up_start, cx4_top + 6, y_up_end - 4, color = sw_col, linewidth = swref_stroke)
    text!(ax, "Reflected " * fmt(sw_ref), position = (cx4_top + 18, y_up_end - 8), align = (:left, :center), color = textcol, fontsize = 13)

    # Sensible
    sens_x_bot = (col_bottom_x[1] + col_bottom_x[3]) * 0.44
    sens_x_top = sens_x_bot + 2
    draw_arrow!(ax, sens_x_bot, y_up_start, sens_x_top - 4, y_sens_end, color = sens_col, linewidth = sens_stroke, headsize = 9.0)
    text!(ax, "Sensible " * fmt(sens), position = (sens_x_top + 8, y_sens_end - 8), align = (:left, :center), color = textcol, fontsize = 13)

    # Latent
    lat_x_bot = (col_bottom_x[1] + col_bottom_x[3]) * 0.66
    lat_x_top = lat_x_bot - 2
    draw_arrow!(ax, lat_x_bot, y_up_start, lat_x_top + 4, y_lat_end, color = lat_col, linewidth = lat_stroke, headsize = 9.0)
    text!(ax, "Latent " * fmt(lat), position = (lat_x_top + 8, y_lat_end - 6), align = (:left, :center), color = textcol, fontsize = 13)

    text!(ax, tlabel, position = (W - 36, atm_y + 36), align = (:right, :center), fontsize = 14)
end

# -----------------------------
# Render PNG frames (GLMakie)
# -----------------------------
...
for i in 1:nframes
    figf = Figure(resolution = (W, H), fontsize = 14)        # GLMakie: can use resolution kw
    axf = Axis(figf[1, 1];
        limits = (0, W, 0, H),
        yreversed = true,
    ...
    save(pngfile, figf)       # GLMakie supports save()
    # close window if shown (we didn't show it), free resources
    figf = nothing
    GC.gc()
...
# -----------------------------
# Stitch PNGs into GIF / MP4 (robust)
# -----------------------------
...
if isempty(png_files)
    ...
    elseif ffmpeg_path !== nothing
        ...
        try
            run(`$ffmpeg_path -y -framerate 8 -i $frames_dir/%04d.png -c:v libx264 -pix_fmt yuv420p $mp4_out`)

#!/usr/bin/env julia
# trenberth_columns_v15.jl
# Fix: sensible/latent point upward to ~half atmosphere; make SW & LW arrow shafts thinner.
# Run: julia trenberth_columns_v15.jl
...
# stroke mapping (original wide mapping kept for sensible/latent if needed)
...
# thinner stroke mapping for SW/LW (user asked thinner shafts/heads)
...
# SW extents (top <-> surface)
y_sw_top = y_top
y_sw_end = y_surface_pen
y_ref_start = y_surface_pen
y_ref_end   = y_top

# LW extents now match SW extents (longer shafts)
y_lw_down_start = y_top
y_lw_down_end   = y_surface_pen

y_lw_up_start = y_surface_pen
y_lw_up_end   = y_top

# SENSIBLE & LATENT: now point upwards to about half the atmosphere height
y_sens_start = y_surface_pen
y_sens_end   = atm_y + atm_h * 0.50   # ~50% atmosphere height (upwards)
y_lat_start  = y_surface_pen
y_lat_end    = atm_y + atm_h * 0.50   # same height but we'll separate horizontally
...
<defs>
  ...
  <!-- normal-sized markers but SW/LW shafts will be thinner -->
...
# vertical arrow helper with option label_at_tip
...
# ---- LONGWAVE (thinner shafts but long) ----
cx_lw_down = col_centers[2]
vertical_arrow(svg, cx_lw_down, y_lw_down_start, y_lw_down_end, lw_color, stroke_thin(lw_down), "m_lw", "LW Down $(fmt(lw_down))"; dx=-18, anchor="end", label_at_tip=true)

cx_lw_up = col_centers[1]
vertical_arrow(svg, cx_lw_up, y_lw_up_start, y_lw_up_end, lw_color, stroke_thin(lw_up), "m_lw", "LW Up $(fmt(lw_up))"; dx=-18, anchor="end", label_at_tip=true)

# ---- SHORTWAVE (thinner shafts) ----
cx_sw_down = col_centers[3]; cx_sw_up = col_centers[4]
vertical_arrow(svg, cx_sw_down, y_sw_top, y_sw_end, sw_color, stroke_thin(sw_down), "m_sw", "SW Down $(fmt(sw_down))"; dx=14, anchor="start")
vertical_arrow(svg, cx_sw_up, y_ref_start, y_ref_end, sw_color, stroke_thin(sw_ref; vmax=280), "m_sw", "Reflected $(fmt(sw_ref))"; dx=14, anchor="start")
...
# ---- SENSIBLE & LATENT: upward to ~half atmosphere, label at tip ----
# separate horizontally so they don't overlap
sens_x = (col_centers[1] + col_centers[3]) * 0.40
lat_x  = (col_centers[1] + col_centers[3]) * 0.60
...
println(svg, "<text x='14' y='$(H - 16)' font-family='Verdana, Arial, sans-serif' font-size='$small_fs' fill='#666666'>Generated: $nowstr — trenberth_columns_v15 (random example)</text>")

println(svg, "</svg>")

# write out files
outname = "trenberth_columns_v15.svg"
open(outname,"w") do io
  write(io, String(take!(svg)))
end
println("Wrote $outname")

pngname = "trenberth_columns_v15.png"
try

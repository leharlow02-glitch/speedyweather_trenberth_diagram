#!/usr/bin/env julia
# Clean CairoMakie animation of Trenberth diagram
# Robust across Makie versions — no clear!/close!, no color macro, no sprintf issues
...
function draw_arrow!(ax, x1, y1, x2, y2; color=parse(Colorant, "#000000"), linewidth=4.0, headsize=10.0)
    lines!(ax, [x1, x2], [y1, y2], linewidth=linewidth, color=color, linecap=:round)
    dx = x2 - x1; dy = y2 - y1
    L = hypot(dx, dy)
    if L < 1e-6
        return
    ...
    px, py = -uy, ux
    tipx, tipy = x2, y2
    b1x = tipx - headsize * (ux + 0.4 * px); b1y = tipy - headsize * (uy + 0.4 * py)
    b2x = tipx - headsize * (ux - 0.4 * px); b2y = tipy - headsize * (uy - 0.4 * py)
...
xs2 = vcat(xs, [right_top_x, right_top_x, left_top_x])
ys2 = vcat(ys, [right_top_y+surf_h, right_top_y+surf_h, left_top_y])
...
function draw_on_axis!(ax, i)
    ...
    for j in 1:4
        ...
        poly!(ax,[x1,x2,x3,x4],[y1,y2,y3,y4],
              color=fillcol, strokecolor=strokecol, strokewidth=2.4)
    end
    ...
    # labels
    text!(ax,"Atmosphere",position=(atm_x+18,atm_y+40),
         align=(:left,:center),color=textcol,fontsize=22)
    text!(ax,"Surface",position=(surf_x+18,surf_y+44),
         align=(:left,:center),color=textcol,fontsize=22)
    ...
    draw_arrow!(ax, cx1_bot, y_up_start, cx1_top+s, y_up_end,
        color=lw_col, linewidth=lw_stroke)
    text!(ax,"LW Up "*fmt(lw_up),
         position=(cx1_top+s-44,y_up_end-6),align=(:right,:center),
         color=textcol,fontsize=13)

    # LW Down
    cx2_top = col_top_x[2] + thin_w*0.5 - s
    cx2_bot = col_bottom_x[2] + thin_w*0.3 - s
    draw_arrow!(ax, cx2_top-s, y_down_start, cx2_bot-4, y_down_end,
        color=lw_col, linewidth=lwd_stroke)
    text!(ax,"LW Down "*fmt(lw_down),
         position=(cx2_bot-46,y_down_end+16),
         align=(:right,:center),color=textcol,fontsize=13)

    # SW Down
    cx3_top = col_top_x[3] + thin_w*0.5 - 6
    cx3_bot = col_bottom_x[3] + thin_w*0.6 - 12
    draw_arrow!(ax,cx3_top-6,y_down_start,cx3_bot-12,y_down_end+2,
        color=sw_col,linewidth=sw_stroke)
    text!(ax,"SW Absorbed "*fmt(sw_down-sw_ref),
         position=(cx3_bot-6,y_down_end+24),
         align=(:right,:center),color=textcol,fontsize=13)

    # SW Reflected
    cx4_bot = col_bottom_x[4] + thin_w*0.5 + 6
    cx4_top = col_top_x[4] + thin_w*0.5 + 10
    draw_arrow!(ax,cx4_bot+2,y_up_start,cx4_top+6,y_up_end-4,
        color=sw_col,linewidth=swref_stroke)
    text!(ax,"Reflected "*fmt(sw_ref),
         position=(cx4_top+18,y_up_end-8),
         align=(:left,:center),color=textcol,fontsize=13)

    # Sensible
    sens_x_bot = (col_bottom_x[1]+col_bottom_x[3])*0.44
    sens_x_top = sens_x_bot + 2
    draw_arrow!(ax,sens_x_bot,y_up_start,sens_x_top-4,y_sens_end,
        color=sens_col,linewidth=sens_stroke,headsize=9.0)
    text!(ax,"Sensible "*fmt(sens),
         position=(sens_x_top+8,y_sens_end-8),
         align=(:left,:center),color=textcol,fontsize=13)

    # Latent
    lat_x_bot = (col_bottom_x[1]+col_bottom_x[3])*0.66
    lat_x_top = lat_x_bot - 2
    draw_arrow!(ax,lat_x_bot,y_up_start,lat_x_top+4,y_lat_end,
        color=lat_col,linewidth=lat_stroke,headsize=9.0)
    text!(ax,"Latent "*fmt(lat),
         position=(lat_x_top+8,y_lat_end-6),
         align=(:left,:center),color=textcol,fontsize=13)

    # frame label
    text!(ax, tlabel, position=(W-36, atm_y+36),
...
if isempty(png_files)
    ...
    # try to sort by numeric index in filename (frame_0001.png, frame_1.png, etc.)

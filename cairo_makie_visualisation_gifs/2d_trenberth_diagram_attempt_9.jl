#!/usr/bin/env julia
# trenberth_columns_v4.jl
# Thinner angled columns that overlap at the surface (SW down/up meet, LW up/down meet),
# sensible/latent without columns, OLR removed for now.
# Run: julia trenberth_columns_v4.jl
...
atm_net     = (lw_up + sw_ref) - (lw_down + sw_ref)   # simple placeholder

fmt(x) = @sprintf("%.1f W/m²", x)

# -----------------------
# Canvas & layout (compact)
# -----------------------
W = 1280
H = 880
pad = 56
...
# make columns thinner so pairs can overlap at surface
thin_w = atm_w * 0.10   # narrow columns
gap = (atm_w - 4*thin_w) / 5.0

# bottom x for 4 thin columns (left->right): LW column pair (up+down), SW column pair (down+up)
# We'll arrange so LW pair on left, SW pair on right (but they meet at surface overlapping)
col_bottom_x = [
    atm_x + gap,                          # LW column (left)
    atm_x + gap*2 + thin_w,               # LW partner (slightly right, overlaps at surface)
    atm_x + gap*3 + thin_w*2,             # SW column (down)
    atm_x + gap*4 + thin_w*3              # SW partner (up)
]

# small top slant offsets to make them angled toward center
slant_px = 36
col_top_x = [
    col_bottom_x[1] + slant_px,
    col_bottom_x[2] - slant_px,
    col_bottom_x[3] - slant_px,
    col_bottom_x[4] + slant_px
]

# column polygon helper and centers
...
# thickness mapping
function stroke_for(val; vmin=0.0, vmax=450.0, wmin=4.0, wmax=26.0)
...
# arrow heights (corrected):
# LW Up: surface -> ~60% up
# LW Down: ~92% -> surface (down)
# SW Down: top -> surface (down)
# SW Reflected: surface -> top (up)
# Sensible/Latent short arrows (no columns) placed between LW and SW stacks
...
# sensible/latent short heights (half-ish)
y_sens_start = y_surface_pen
y_sens_end   = atm_y + atm_h*0.32
y_lat_start  = y_surface_pen
y_lat_end    = atm_y + atm_h*0.44
...
# angled thin columns (subtle fill)
for i in 1:4
    coords = col_polygon_coords(i)
    println(svg, "<polygon points='$(coords[1]) $(coords[2]), $(coords[3]) $(coords[4]), $(coords[5]) $(coords[6]), $(coords[7]) $(coords[8])' fill='#497fa1' fill-opacity='0.06' stroke='none'/>")
end
...
# helper to draw straight vertical arrow centered on given x
function draw_arrow(buf, cx, y1, y2, color, strokew, marker_id, label; label_dx=14, label_dy=-4, anchor="start")
    # always draw as line y1->y2 with marker-end; arrow direction depends on whether y2>y1
...
# ---- Longwave pair (columns 1 & 2 overlap at surface)
cx_lw_up   = col_centers[1]         # left column centre for LW up
cx_lw_down = col_centers[2]         # partner column centre (overlaps at surface)
# LW Up (from surface up)
draw_arrow(svg, cx_lw_up, y_lw_up_start, y_lw_up_end, lw_color, stroke_for(lw_up), "m_lw", "LW Up $(fmt(lw_up))"; label_dx=-12, anchor="end")
# LW Down (from high atm down)
draw_arrow(svg, cx_lw_down, y_lw_down_start, y_lw_down_end, lw_color, stroke_for(lw_down), "m_lw", "LW Down $(fmt(lw_down))"; label_dx=-12, anchor="end", label_dy=10)

# ---- Shortwave pair (columns 3 & 4 overlap at surface)
cx_sw_down = col_centers[3]
cx_sw_up   = col_centers[4]
# SW Down (top -> surface)
draw_arrow(svg, cx_sw_down, y_sw_top, y_sw_end, sw_color, stroke_for(sw_down), "m_sw", "SW Down $(fmt(sw_down))"; label_dx=12, anchor="start")
# SW Reflected (surface -> up)
draw_arrow(svg, cx_sw_up, y_ref_start, y_ref_end, sw_color, stroke_for(sw_ref; vmax=280), "m_sw", "Reflected $(fmt(sw_ref))"; label_dx=12, anchor="start")

# place SW Absorbed label slightly left of the overlap so it doesn't collide with LW Down
println(svg, "<text x='$( (cx_sw_down + cx_sw_up)/2 - 46 )' y='$(y_sw_end + 12)' font-family='Verdana, Arial, sans-serif' font-size='$flux_fs' fill='$text_color' text-anchor='end'>SW Absorbed $(fmt(sw_abs))</text>")

# ---- Sensible & Latent (no column) placed between LW and SW stacks
sens_x = (col_centers[1] + col_centers[3]) / 2   # mid between LW and SW stacks
# make sensible & latent thin and short, labels to right
draw_arrow(svg, sens_x, y_sens_start, y_sens_end, sens_color, stroke_for(h_val; vmax=60), "m_sens", "Sensible $(fmt(h_val))"; label_dx=12, anchor="start")
draw_arrow(svg, sens_x + 14, y_lat_start, y_lat_end, lat_color, stroke_for(le_val; vmax=240), "m_lat", "Latent $(fmt(le_val))"; label_dx=12, anchor="start", label_dy=10)

# Atmosphere & surface net labels (tightened)
...
println(svg, "<text x='12' y='$(H - 12)' font-family='Verdana, Arial, sans-serif' font-size='$small_fs' fill='#666666'>Generated: $nowstr — trenberth_columns_v4 (random example)</text>")

println(svg, "</svg>")

# write files
outname = "trenberth_columns_v4.svg"
...
pngname = "trenberth_columns_v4.png"
try
    run(`rsvg-convert -o $pngname $outname`)
    println("Also wrote $pngname (via rsvg-convert)")
catch e
    println("rsvg-convert not available or failed — SVG produced; open it in a browser.")
end

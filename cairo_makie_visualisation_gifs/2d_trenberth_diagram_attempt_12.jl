#!/usr/bin/env julia
# trenberth_columns_v9.jl
# Adjust LW columns right, match LW extents to SW (for now), shorten sensible/latent,
# add lapse-rate ticks, tidy label positions.
# Run: julia trenberth_columns_v9.jl
...
# thin columns and aggressive separation (LW left, SW right)
thin_w = atm_w * 0.075    # narrow columns
left_cluster_x = atm_x + 48
# move LW pair slightly right a bit so tops not clipped
left_cluster_x += 18

# right cluster anchored further right
right_cluster_x = atm_x + atm_w - 48 - thin_w*2 - 14

# bottom positions for columns (LW pair near left_cluster_x, SW pair near right_cluster_x)
col_bottom_x = [
  left_cluster_x,                     # LW left bottom
  left_cluster_x + thin_w*0.9 + 10.0, # LW right bottom (close)
  right_cluster_x,                    # SW left bottom (right cluster)
  right_cluster_x + thin_w*0.9 + 10.0 # SW right bottom (partner)
]

# LW V-shape slant; SW slight inward slant
slant_px = 70
col_top_x = [
...
# arrow heights: match LW to SW style:
# - SW Down: top -> surface
# - SW Reflected: surface -> top
# - LW Down: top -> surface (visible)
# - LW Up: surface -> mid atmosphere (so visually similar to SW Up/Down)
y_lw_up_start = y_surface_pen
y_lw_up_end   = atm_y + atm_h * 0.60  # up to ~60%
...
# sensible/latent: much shorter (50% -> now ~25% of previous), separated more
y_sens_start = y_surface_pen
y_sens_end   = atm_y + atm_h * 0.10   # very short
y_lat_start  = y_surface_pen
y_lat_end    = atm_y + atm_h * 0.16
...
# lapse-rate ticks (left inside atmosphere)
tick_x = atm_x + 18
tick_len = 10
n_ticks = 6
for k in 1:n_ticks
  yy = atm_y + 20 + (k-1)*(atm_h - 40)/(n_ticks-1)
  println(svg, "<line x1='$(tick_x)' y1='$yy' x2='$(tick_x + tick_len)' y2='$yy' stroke='#6b7f88' stroke-width='1' stroke-dasharray='2,3'/>")
end
println(svg, "<text x='$(tick_x + tick_len + 6)' y='$(atm_y + 20)' font-family='Verdana, Arial, sans-serif' font-size='12' fill='#6b7f88'>Lapse-rate guide</text>")
...
# arrow helper (vertical with marker-end)
...
# ---- Longwave pair (V-shaped) - left cluster
cx_lw_up = col_centers[1]
cx_lw_down = col_centers[2]
vertical_arrow(svg, cx_lw_up, y_lw_up_start, y_lw_up_end, lw_color, stroke_for(lw_up), "m_lw", "LW Up $(fmt(lw_up))"; dx=-16, anchor="end")
vertical_arrow(svg, cx_lw_down, y_lw_down_start, y_lw_down_end, lw_color, stroke_for(lw_down), "m_lw", "LW Down $(fmt(lw_down))"; dx=-16, anchor="end", dy=10)

# ---- Shortwave pair (right cluster)
...
# SW Absorbed label centered over overlap
...
# ---- Sensible & Latent: short & separated more widely
sens_x = (col_centers[1] + col_centers[3]) * 0.46   # left of center
lat_x  = (col_centers[1] + col_centers[3]) * 0.62   # right of center
vertical_arrow(svg, sens_x, y_sens_start, y_sens_end, sens_color, stroke_for(h_val; vmax=60), "m_sens", "Sensible $(fmt(h_val))"; dx=12)
vertical_arrow(svg, lat_x, y_lat_start, y_lat_end, lat_color, stroke_for(le_val; vmax=240), "m_lat", "Latent $(fmt(le_val))"; dx=12, dy=8)
...
println(svg, "<text x='12' y='$(H - 12)' font-family='Verdana, Arial, sans-serif' font-size='$small_fs' fill='#666666'>Generated: $nowstr — trenberth_columns_v9 (random example)</text>")

println(svg, "</svg>")

# write
outname = "trenberth_columns_v9.svg"
...
# try PNG
pngname = "trenberth_columns_v9.png"
try

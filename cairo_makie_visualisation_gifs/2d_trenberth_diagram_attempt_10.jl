#!/usr/bin/env julia
# trenberth_columns_v6.jl
# Stronger overlap at surface, visible LW Down line, V-shaped LW columns, shorter sensible/latent.
# Run: julia trenberth_columns_v6.jl
...
# Make columns *thinner* but move bottoms much closer for larger overlap
thin_w = atm_w * 0.08    # narrower columns than before
# reduce gaps so bottom columns almost touch / overlap at surface
gap = (atm_w - 4*thin_w) / 7.0   # smaller gaps increases overlap

# bottom x positions for columns (LW-left, LW-right; SW-left, SW-right)
col_bottom_x = [
  atm_x + gap,                                   # LW left bottom
  atm_x + gap + thin_w*0.9 + thin_w*0.08,        # LW right bottom placed close -> overlap
  atm_x + gap*3 + thin_w*1.2,                    # SW left bottom
  atm_x + gap*4 + thin_w*2.0                     # SW right bottom (partner)
]

# enforce a stronger V for LW pair: top x move towards center
slant_px = 70
col_top_x = [
  col_bottom_x[1] - slant_px,                    # LW left top (move left)
  col_bottom_x[2] + slant_px,                    # LW right top (move right) => clear V
  col_bottom_x[3] - (slant_px/3),                # SW slight inward slant
  col_bottom_x[4] + (slant_px/3)
]
...
# vertical positions for arrows
y_top = atm_y + 18
y_surface_pen = surf_y + 12

# arrow heights
y_lw_up_start = y_surface_pen
y_lw_up_end   = atm_y + atm_h * 0.62

y_lw_down_start = atm_y + atm_h * 0.96    # push even higher start to emphasize downwards
...
# sensible/latent short heights (center gap)
y_sens_start = y_surface_pen
y_sens_end   = atm_y + atm_h * 0.28    # shorter than before
y_lat_start  = y_surface_pen
y_lat_end    = atm_y + atm_h * 0.40
...
# ---- Longwave pair (V-shaped) - stronger V and visible LW Down
...
# ---- Shortwave pair (more overlap at surface)
...
# SW Absorbed label centered into overlap
mid_sw = (cx_sw_down + cx_sw_up)/2
println(svg, "<text x='$(mid_sw - 30)' y='$(y_sw_end + 14)' font-family='Verdana, Arial, sans-serif' font-size='$flux_fs' fill='$text_color' text-anchor='end'>SW Absorbed $(fmt(sw_abs))</text>")

# ---- Sensible & Latent centered between stacks (shorter lengths)
mid_gap_x = (col_centers[1] + col_centers[3])/2
vertical_arrow(svg, mid_gap_x, y_sens_start, y_sens_end, sens_color, stroke_for(h_val; vmax=60), "m_sens", "Sensible $(fmt(h_val))"; dx=12)
vertical_arrow(svg, mid_gap_x + 14, y_lat_start, y_lat_end, lat_color, stroke_for(le_val; vmax=240), "m_lat", "Latent $(fmt(le_val))"; dx=12, dy=8)
...
println(svg, "<text x='12' y='$(H - 12)' font-family='Verdana, Arial, sans-serif' font-size='$small_fs' fill='#666666'>Generated: $nowstr — trenberth_columns_v6 (random example)</text>")

println(svg, "</svg>")

# write
outname = "trenberth_columns_v6.svg"
...
# try PNG
pngname = "trenberth_columns_v6.png"
try
